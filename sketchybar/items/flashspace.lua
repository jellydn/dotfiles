	...
	for _, workspace in ipairs(cached_workspaces) do
		if workspace.name == workspace_name then
			if workspace.icon_path then
				local icon_map = {
					["Cursor.icns"] = "􀤋", -- IDE/code editor icon
					["Code - Insiders.icns"] = "􀤋",
					["Zed Preview.icns"] = "􀤋",
					["AppIcon.icns"] = "􀪏", -- Ghostty terminal
					["electron.icns"] = "􀦳", -- Slack (hashtag.circle.fill)
					["app.icns"] = "􀎬", -- Brave Browser
					["firefox.icns"] = "􀎬", -- Zen Browser
					["icon.icns"] = "􀍕", -- Spark Desktop
					["messenger.icns"] = "􀌤", -- Messenger
					["Icon.icns"] = "􀑪", -- Spotify
					["Navicat Premium Lite.icns"] = "􀫕", -- Navicat (cylinder.split.1x2.fill)
					["ZPLogo.icns"] = "􀌤", -- Zoom
				}
